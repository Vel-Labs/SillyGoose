...
import { assignAiPlayerTwo, createRoom, joinRoom, requestRematch } from "@/lib/game/engine";
...
export async function POST(request: Request) {
  ...
  const { roomId, join, reset, aiMode, aiPlayerTwo, goose } = await request.json().catch(() => ({}));
  try {
    const result = reset
    ...
    const room = result
      ? result.room
      : aiPlayerTwo
        ? await assignAiPlayerTwo(String(roomId), user.id)
        : join
        ? await joinRoom(String(roomId), user.id, String(goose || "jefe"))
        : await createRoom(user.id, Boolean(aiMode), String(goose || "captain"));
    await addAudit(reset ? `${user.name} requested a rematch in ${room.id}.` : aiPlayerTwo ? `${user.name} assigned AI Goose as Player Two in ${room.id}.` : join ? `${user.name} joined ${room.id} as a verified Player Two.` : `${user.name} opened ${room.id}.`, "game");
